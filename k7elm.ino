...
SoftwareSerial dlcSer(13, 13, false, 256);		// GPIO13 K-line
...
int dlcCommand(byte cmd, byte num, byte loc, byte len, byte data[]) 
{
  ...
  int idlechk1 = digitalRead(13); delay(1);
  int idlechk2 = digitalRead(13);
  
  if (!(idlechk1 && idlechk2)) { return 0; }	// line IDLE chk
  ...
  memset(data, 0, sizeof(data));
	...
	dlcSer.enableTx(true);
 
	dlcSer.write(cmd);  // header/cmd read memory ??
	dlcSer.write(num);  // num of bytes to send
	dlcSer.write(loc);  // address
	dlcSer.write(len);  // num of bytes to read
	dlcSer.write(crc);  // checksum

	dlcSer.enableTx(false);
	 
  int i = 0;
  while (i < (len+3) && millis() < timeOut) {
    if (dlcSer.available()) { data[i] = dlcSer.read(); i++; }
  }
  if (data[0] != 0x00 && data[1] != (len+3)) { return 0; }	// or use checksum?
  if (i < (len+3)) { return 0; } 							// timeout ERROR
  return 1; 												// success
}
...
void setup()
{
  ...
  dlcSer.begin(9600);				// DLC port		Try 9470!?		
...
void loop()
{
  ...
  {
			Serial.println("\n[Client connected]");
			sprintf_P(wfdata2, PSTR("K7ELM v0.1\r\n>"));
			j=0;
			while (wfdata2[j] != '\0') {
				client.print(wfdata2[j]); Serial.write(wfdata2[j]); j++;
			}
	while (	client.connected()) {	
    while ( client.available()) {
      wfdata1[i] = toupper(client.read());
      
      Serial.print(wfdata1[i]); // debug
